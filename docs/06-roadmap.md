...
## Хвосты, требующие sudo/аккаунтов владельца

Все три пункта ниже закрывает один скрипт: `ssh -t manshoo.ru "sudo bash ~/nginx-hardening.sh"`.

- [ ] basic auth на `/django-admin` — вторая дверь перед формой входа Django: путь общеизвестный, ботам-переборщикам он знаком, а своего лимита попыток у Django-админки нет (в отличие от `/api/auth/login`).
- [ ] `www.manshoo.ru` → 301 на apex (сейчас сайт открывается по двум адресам с кодом 200 — дубль для поисковиков; canonical прикрывает, но редирект честнее).
- [ ] HTTP/2 (сейчас HTTP/1.1).
